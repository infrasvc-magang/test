...
def delete(title):
    presentation = PresentationHistory.query.filter_by(title=title).first()
    
    if not presentation:
        return jsonify({"error": "Presentation not found"}), 404

    # Extract the filename from the download link
    filename = presentation.download_link.split('/')[-1]
    file_path = os.path.abspath(os.path.join('PPTX', filename))
